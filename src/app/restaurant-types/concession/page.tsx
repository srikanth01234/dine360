...
import { Check, Zap, Star, Clock, Target, Utensils, Users, Layout } from 'lucide-react'
  ...
  return (
    <div className="min-h-screen bg-white font-sans text-zinc-900">
        ...
        <div className="grid lg:grid-cols-2 gap-10 items-center">
          <div className="flex flex-col gap-6 z-10">
            <div className="flex -space-x-2 overflow-hidden items-center mb-2">
              {[1, 2, 3].map((i) => (
                <div key={i} className="inline-block h-8 w-8 rounded-full ring-2 ring-white bg-zinc-200 overflow-hidden relative">
                  <Image src={`https://randomuser.me/api/portraits/thumb/women/${i + 52}.jpg`} alt="User" fill className="object-cover" />
                </div>
              ))}
              <span className="ml-3 text-zinc-600 text-xs font-medium bg-zinc-100 px-2.5 py-1 rounded-full text-nowrap tracking-tight">Choice of 1,200+ Concession Stands</span>
            </div>
            ...
            <h1 className="text-4xl sm:text-5xl md:text-6xl font-bold tracking-tight text-zinc-900 leading-[1.1]">
              Scale Your <br />
              <span className="italic font-serif font-light text-red-600">Concession Performance</span>
            </h1>
            <p className="text-lg text-zinc-600 max-w-lg leading-relaxed">
              Power large-scale concession stands with a system that handles high guest volume and fast checkout.
            </p>
          ...
          <div className="relative h-[500px] w-full flex items-center justify-center lg:justify-end" style={{ perspective: '1000px' }}>
            ...
            <div className="relative w-full h-full max-w-md mx-auto lg:mr-0">
              <motion.div className="absolute top-20 left-0 w-56 h-72 bg-white rounded-3xl shadow-xl overflow-hidden border-4 border-white transform origin-bottom-right" initial={{ opacity: 0, x: -50, rotate: -20 }} animate={{ opacity: 1, x: 0, rotate: -12 }} transition={{ duration: 0.8, delay: 0.2 }}>
                <Image src={CARD_IMAGES[1]} alt="Snack Service" fill className="object-cover" />
              ...
              <motion.div className="absolute top-10 left-12 md:left-20 w-64 h-80 bg-white rounded-3xl shadow-2xl overflow-hidden border-4 border-white z-20" initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
                <Image src={CARD_IMAGES[0]} alt="Popcorn and Drinks" fill className="object-cover" />
                <div className="absolute bottom-0 left-0 right-0 h-2/5 bg-gradient-to-t from-zinc-950/90 via-zinc-950/40 to-transparent p-4 flex flex-col justify-end text-white text-nowrap">
      ...
      <section className="py-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto bg-zinc-50 rounded-[3rem]">
        <div className="text-center max-w-4xl mx-auto mb-16 text-zinc-900">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="flex items-center justify-center gap-2 mb-6 text-nowrap">
            ...
            <span className="text-red-600 font-bold tracking-widest uppercase text-xs">HIGH-SPEED LANDING TECH</span>
            <div className="h-[1px] w-8 bg-red-600" />
          </motion.div>
          <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 }} className="text-3xl md:text-5xl font-bold mb-6 leading-tight">
            Streamlined <span className="text-red-600 font-serif italic">Concession Sales Efficiency</span>.
          </motion.h2>
          <p className="text-base text-zinc-600 max-w-2xl mx-auto">
            Dine360 provides the reliability and features that concession stand owners need to manage high guest volume during peaks.
          </p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-5xl mx-auto">
          {[
            { label: "Faster Guest Processing", value: "90%", desc: "Cut queue times with high-speed POS terminals." },
            { label: "Increase Order Volume", value: "95%", desc: "Maximize output and efficiency per station." },
            { label: "Zero Bottlenecks", value: "100%", desc: "Reliable transaction processing during event peaks." }
          ].map((stat, i) => (
      ...
      <section className="py-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        <div className="grid lg:grid-cols-2 gap-12 items-center text-zinc-900 font-nowrap">
          <div className="flex flex-col gap-6 order-2 lg:order-1 text-nowrap">
            <div className="flex items-center gap-3">
              ...
              <span className="text-red-600 font-bold tracking-wider uppercase text-xs">EVENT-SYNCED FEATURES</span>
            </div>
            <h2 className="text-3xl md:text-4xl font-bold leading-tight">High-Speed Checkout Workflow</h2>
            <ul className="space-y-4">
              {[
                "Highly intuitive POS with lightning-fast order entry for simple menus",
                "Advanced customer loyalty and digital points for repeat event-goers",
                "Optimized multi-lane checkout for peak event windows and bursts",
                "Offline mode ensures service never stops during high-traffic intervals"
              ].map((point, i) => (
                <li key={i} className="flex gap-3 items-start group">
                  ...
                  <span className="text-zinc-600 text-sm leading-relaxed whitespace-normal">{point}</span>
                </li>
              ))}
            </ul>
          </div>
          <div className="relative h-[400px] w-full order-1 lg:order-2">
            <motion.div className="absolute inset-0 bg-white rounded-3xl shadow-xl overflow-hidden border border-zinc-100 p-6" initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }}>
              <Image src={CARD_IMAGES[3]} alt="Stadium Concession" fill className="object-cover rounded-2xl" />
            </motion.div>
          </div>
        </div>
      </section>
