...
import { Check, Zap, RefreshCw, Clock, Target, Utensils, Users, Building } from 'lucide-react'
...
const CafeteriaPage = () => {
  return (
    <div className="min-h-screen bg-white">
      ...
      <main className="pt-24 pb-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto overflow-hidden text-zinc-800">
        <div className="grid lg:grid-cols-2 gap-10 items-center">
          <div className="flex flex-col gap-6 z-10">
            <div className="flex -space-x-2 overflow-hidden items-center mb-2">
              ...
              <span className="ml-3 text-zinc-600 text-xs font-medium bg-zinc-100 px-2.5 py-1 rounded-full">Used by 800+ Institutional Cafeterias</span>
            </div>
            ...
            <h1 className="text-4xl sm:text-5xl md:text-6xl font-bold tracking-tight text-zinc-900 leading-[1.1]">
              Scale Your <br />
              <span className="italic font-serif font-light text-zinc-800 text-nowrap">Cafeteria Operations</span>
            </h1>
            <p className="text-lg text-zinc-600 max-w-lg leading-relaxed">
              Power large-scale dining with a system that handles high guest volume and fast checkout.
            </p>
            <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center mt-1">
              <Link href="/contact" className="bg-red-600 hover:bg-red-700 text-white px-7 py-3.5 rounded-xl font-semibold text-base transition-all shadow-lg shadow-red-600/20 active:scale-95 text-nowrap">Shop Now</Link>
              <Link href="/contact" className="bg-black hover:bg-zinc-900 text-white px-7 py-3.5 rounded-xl font-semibold text-base transition-all active:scale-95 text-nowrap">Book A Demo</Link>
            </div>
          </div>
          ...
          <div className="relative h-[500px] w-full flex items-center justify-center lg:justify-end" style={{ perspective: '1000px' }}>
            ...
            <div className="relative w-full h-full max-w-md mx-auto lg:mr-0">
              <motion.div className="absolute top-20 left-0 w-56 h-72 bg-white rounded-3xl shadow-xl overflow-hidden border-4 border-white transform origin-bottom-right" initial={{ opacity: 0, x: -50, rotate: -20 }} animate={{ opacity: 1, x: 0, rotate: -12 }} transition={{ duration: 0.8, delay: 0.2 }}>
                <Image src={CARD_IMAGES[1]} alt="Meal Tray" fill className="object-cover" />
                <div className="absolute bottom-0 left-0 right-0 h-2/5 bg-gradient-to-t from-zinc-900/90 via-zinc-900/40 to-transparent p-4 flex flex-col justify-end">
                  <p className="text-[10px] text-zinc-300 font-medium tracking-wide uppercase mb-0.5">Guest Volume</p>
              ...
              <motion.div className="absolute top-10 left-12 md:left-20 w-64 h-80 bg-white rounded-3xl shadow-2xl overflow-hidden border-4 border-white z-20" initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
                <Image src={CARD_IMAGES[0]} alt="Salad Bar" fill className="object-cover" />
                <div className="absolute bottom-0 left-0 right-0 h-2/5 bg-gradient-to-t from-zinc-900/90 via-zinc-900/40 to-transparent p-4 flex flex-col justify-end">
                  <p className="text-[10px] text-zinc-300 font-medium tracking-wide uppercase mb-0.5">Checkout Speed</p>
      ...
      <section className="py-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto bg-zinc-50/50 rounded-[3rem]">
        <div className="text-center max-w-4xl mx-auto mb-16 text-zinc-900">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="flex items-center justify-center gap-2 mb-6">
            ...
            <span className="text-red-600 font-bold tracking-widest uppercase text-xs text-nowrap">HIGH-VOLUME STATIONS</span>
            <div className="h-[1px] w-8 bg-red-500" />
          </motion.div>
          <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 }} className="text-3xl md:text-5xl font-bold mb-6 leading-tight">
            Streamlined <span className="text-red-700 font-serif italic">Cafeteria Efficiency</span>.
          </motion.h2>
          <p className="text-base text-zinc-600 max-w-2xl mx-auto">
            Our specialized cafeteria solution is designed for busy institutional and commercial dining halls.
          </p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-5xl mx-auto">
          {[
            { label: "Faster Guest Processing", value: "90%", desc: "Reduce wait times with intuitive POS systems." },
            { label: "Increase Order Volume", value: "95%", desc: "Maximize output and efficiency per station." },
            { label: "Zero Checkout Bottlenecks", value: "100%", desc: "Reliable, high-speed payment processing." }
          ].map((stat, i) => (
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <div className="flex flex-col gap-6 order-2 lg:order-1 text-zinc-900">
            <div className="flex items-center gap-3">
              ...
              <span className="text-red-600 font-bold tracking-wider uppercase text-xs text-nowrap">RELIABLE OPERATIONS</span>
            </div>
            <h2 className="text-3xl md:text-4xl font-bold leading-tight">Smooth Bulk Dining</h2>
            <ul className="space-y-4">
              {[
                "Speedy POS terminals for lightning-fast self-service or staff-assisted checkout",
                "Advanced inventory management to minimize food waste in bulk cooking",
                "Integration with pre-paid card systems and employee meal plans",
                "Self-service kiosks to reduce staff overhead and speed up orders"
              ].map((point, i) => (
          ...
          <div className="relative h-[400px] w-full order-1 lg:order-2">
            <motion.div className="absolute inset-0 bg-white rounded-3xl shadow-xl overflow-hidden border border-zinc-100 p-6" initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }}>
              <Image src={CARD_IMAGES[3]} alt="Kitchen Prep" fill className="object-cover rounded-2xl" />
            </motion.div>
          </div>
        </div>
      </section>
      ...
      <section className="py-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto bg-zinc-900 rounded-[3rem] text-white overflow-hidden relative">
        <div className="absolute top-0 left-0 w-full h-full bg-linear-to-br from-red-600/10 to-transparent opacity-50" />
        <div className="relative z-10 grid lg:grid-cols-2 gap-12 items-center">
          <div>
            <h2 className="text-3xl md:text-4xl font-bold mb-6">Optimized for Scale</h2>
            <div className="space-y-6">
              {[
                { icon: Users, title: "Group Billing", desc: "Easily handle large groups and institutional accounts." },
                { icon: Clock, title: "Rush Hour Mode", desc: "Speed up interface for high-traffic periods." },
                { icon: Building, title: "Multi-Unit Support", desc: "Manage multiple cafeteria locations from one dashboard." }
              ].map((item, i) => (
                <div key={i} className="flex gap-4">
                  <div className="w-12 h-12 rounded-2xl bg-red-600/20 flex items-center justify-center text-red-500 shrink-0">
          ...
          <div className="bg-white/5 rounded-3xl p-8 border border-white/10 backdrop-blur-sm">
            <h3 className="text-2xl font-bold mb-6 text-center">Ready to Upgrade?</h3>
            <div className="space-y-4">
              <Link href="/contact" className="block w-full py-4 bg-red-600 hover:bg-red-700 text-center rounded-2xl font-bold transition-colors">Get Started Now</Link>
              <Link href="/contact" className="block w-full py-4 bg-white/10 hover:bg-white/20 text-center rounded-2xl font-bold transition-all border border-white/10">Talk to an Expert</Link>
            </div>
            <p className="text-zinc-500 text-xs text-center mt-6">Free trial available for 14 days. No credit card required.</p>
          </div>
        </div>
      </section>
      <Footer />
