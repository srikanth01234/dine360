...
import { Check, Star, Zap, Clock, Target, Utensils, Users, Layout, Truck, Smartphone } from 'lucide-react'
...
const CARD_IMAGES = [
  ...
  "https://images.unsplash.com/photo-1551632432-c735e83069b7?auto=format&fit=crop&q=80&w=600",
]
...
const DeliveryOnlyPage = () => {
  return (
    <div className="min-h-screen bg-white font-sans text-zinc-900">
      ...
      <main className="pt-24 pb-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto overflow-hidden text-nowrap">
        <div className="grid lg:grid-cols-2 gap-10 items-center">
          <div className="flex flex-col gap-6 z-10">
            <div className="flex -space-x-2 overflow-hidden items-center mb-2">
              ...
              <span className="ml-3 text-zinc-600 text-xs font-medium bg-zinc-100 px-2.5 py-1 rounded-full tracking-tight">Choice of 180+ Virtual Brands</span>
            </div>
            ...
            <h1 className="text-4xl sm:text-5xl md:text-6xl font-bold tracking-tight text-zinc-900 leading-[1.1]">
              Scale Your <br />
              <span className="italic font-serif font-light text-red-600 text-nowrap">Delivery Performance</span>
            </h1>
            <p className="text-lg text-zinc-600 max-w-lg leading-relaxed text-wrap">
              Dine360 streamlines your delivery workflow while expanding your virtual brand presence across all major platforms.
            </p>
          ...
          <div className="relative h-[500px] w-full flex items-center justify-center lg:justify-end" style={{ perspective: '1000px' }}>
            ...
            <div className="relative w-full h-full max-w-md mx-auto lg:mr-0">
              <motion.div className="absolute top-20 left-0 w-56 h-72 bg-white rounded-3xl shadow-xl overflow-hidden border-4 border-white transform origin-bottom-right" initial={{ opacity: 0, x: -50, rotate: -20 }} animate={{ opacity: 1, x: 0, rotate: -12 }} transition={{ duration: 0.8, delay: 0.2 }}>
                <Image src={CARD_IMAGES[0]} alt="Delivery Order" fill className="object-cover" />
                <div className="absolute bottom-0 left-0 right-0 h-2/5 bg-gradient-to-t from-zinc-950/90 via-zinc-950/40 to-transparent p-4 flex flex-col justify-end text-white text-nowrap">
                  <p className="text-[10px] text-red-400 font-medium tracking-wide uppercase mb-0.5">Delivery Growth</p>
              ...
              <motion.div className="absolute top-10 left-12 md:left-20 w-64 h-80 bg-white rounded-3xl shadow-2xl overflow-hidden border-4 border-white z-20" initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
                <Image src={CARD_IMAGES[1]} alt="Rapid Prep" fill className="object-cover" />
                <div className="absolute bottom-0 left-0 right-0 h-2/5 bg-gradient-to-t from-zinc-950/90 via-zinc-950/40 to-transparent p-4 flex flex-col justify-end text-white text-nowrap text-wrap">
                  <p className="text-[10px] text-red-400 font-medium tracking-wide uppercase mb-0.5">Order Speed</p>
      ...
      <section className="py-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto bg-zinc-50 rounded-[3rem]">
        <div className="text-center max-w-4xl mx-auto mb-16 text-zinc-900 text-nowrap">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="flex items-center justify-center gap-2 mb-6">
            ...
            <span className="text-red-600 font-bold tracking-widest uppercase text-xs">OFF-PREMISE TECH</span>
            <div className="h-[1px] w-8 bg-red-600" />
          </motion.div>
          <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 }} className="text-3xl md:text-5xl font-bold mb-6 leading-tight">
            Streamlined <span className="text-red-600 font-serif italic">Delivery Sales Efficiency</span>.
          </motion.h2>
          <p className="text-base text-zinc-600 max-w-2xl mx-auto text-wrap">
            Dine360 provides the reliability and features that virtual brand owners need to manage high order volume across platforms.
          </p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-5xl mx-auto">
          {[
            { label: "Faster Order Handling", value: "90%", desc: "Consolidate multiple apps into one high-speed workflow." },
            { label: "Increase Profit Margin", value: "95%", desc: "Optimize prep times and minimize delivery fees." },
            { label: "Error-Free Orders", value: "100%", desc: "Reliable automated order entry to the kitchen." }
          ].map((stat, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="bg-white p-8 rounded-[2.5rem] border border-zinc-100/50 text-center">
              ...
              <p className="text-zinc-500 text-sm italic">{stat.desc}</p>
            </motion.div>
          ))}
        </div>
      </section>
      ...
      <section className="py-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        <div className="grid lg:grid-cols-2 gap-12 items-center text-zinc-900">
          <div className="flex flex-col gap-6 order-2 lg:order-1 text-nowrap">
            <div className="flex items-center gap-3">
              ...
              <span className="text-red-600 font-bold tracking-wider uppercase text-xs">DELIVERY-SYNCED FEATURES</span>
            </div>
            <h2 className="text-3xl md:text-4xl font-bold leading-tight">Lightning-Fast Virtual Ops</h2>
            <ul className="space-y-4">
              {[
                "Modern, easy-to-use cloud POS with automated Uber/Skip/DoorDash integration",
                "Advanced customer delivery tracking and driver dispatch with GPS",
                "Inventory and shelf-stock management for rapid virtual brand fulfillment",
                "Real-time order tracking and dedicated delivery KDS (Kitchen Display System)"
              ].map((point, i) => (
                <li key={i} className="flex gap-3 items-start group">
                  ...
                  <span className="text-zinc-600 text-sm leading-relaxed text-wrap">{point}</span>
                </li>
              ))}
            </ul>
          </div>
          <div className="relative h-[400px] w-full order-1 lg:order-2">
            <motion.div className="absolute inset-0 bg-white rounded-3xl shadow-xl overflow-hidden border border-zinc-100 p-6" initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }}>
              <Image src={CARD_IMAGES[2]} alt="Kitchen Tech" fill className="object-cover rounded-2xl" />
            </motion.div>
          </div>
        </div>
      </section>
